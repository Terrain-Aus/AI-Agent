...
export function foremanReview(
  ...
  if (!est) {
    return assemble([
      issue('not-priced', 'block', 'Quote not priced yet', "There's no estimate on this quote, so there's nothing for me to check. Price it first.", 'Ask the Apprentice to price the job, then bring it back to me.'),
    ])
  }
  ...
  if (area <= 0) {
    issues.push(issue('no-area', 'block', 'No area / measurement', "There's no area on this job. Without a measured quantity the whole price is a guess.", 'Add the m² (or the measured quantity) on the Job tab and re-price.'))
  }
  if (isConcrete && area > 0 && spec.thicknessMm <= 0) {
    issues.push(issue('no-thickness', 'warn', 'No slab thickness', "You've got a concrete job with no thickness set. Thickness drives concrete volume — the biggest single cost on the pour.", 'Set the slab/pour thickness (mm) on the Job tab.'))
  }

  /* ── Labour ───────────────────────────────────────────────────── */
  if (est.baseCost > 0 && labourTotal <= 0) {
    issues.push(issue('no-labour', 'block', 'No labour in the quote', "There's no labour costed on this job. Materials don't lay themselves — if the crew isn't in the price, you're paying their wages out of your margin.", 'Add the crew (prep, lay & finish, etc.) on the Job tab, or confirm this is supply-only.'))
  }

  /* ── Materials ────────────────────────────────────────────────── */
  if (expectsMaterials && materialsTotal <= 0) {
    issues.push(issue('no-materials', 'block', 'No materials costed', `This is a ${spec.jobType.replace('-', ' ')} job but there are no materials in the price. The supply has to be in here somewhere.`, 'Check the materials on the Job tab and re-price.'))
  }

  /* ── Concrete with no concrete supply ─────────────────────────── */
  if (expectsConcrete && !items('materials').some((i) => /concrete/i.test(i.label))) {
    issues.push(issue('concrete-no-supply', 'block', 'Concrete job, no concrete supply', "You're pouring concrete but I can't see a concrete supply cost. That's the single biggest line on the job — leaving it out guts your price.", 'Confirm the concrete supply is costed (volume × $/m³) and re-price.'))
  }
  ...
  if (producesSpoil && disposalTotal <= 0) {
    ...
    issues.push(issue('dig-no-disposal', sev, 'Spoil with nowhere to go', "You're digging but there's no disposal or spoil handling in the price. That dirt has to be carted and tipped — or noted as cut-to-fill staying on site.", 'Add spoil cartage + tipping fees, or note on the quote that spoil stays on site.'))
  } else if (!producesSpoil && textMentionsRemoval && disposalTotal <= 0) {
    issues.push(issue('removal-text-no-disposal', 'warn', 'Removal mentioned, no disposal', "The job notes mention excavation/demolition/removal, but there's no disposal in the price. Make sure the spoil or rubbish is handled.", 'Add disposal/cartage if material is leaving site, or clarify the scope.'))
  }

  /* ── Plant / equipment where required ─────────────────────────── */
  if (digHappening && machineryTotal <= 0) {
    issues.push(issue('dig-no-plant', 'block', 'Digging with no plant', "There's excavation on this job but no machinery in the price. Hand-digging a job like this isn't real — the excavator/bobcat needs to be costed.", 'Add the plant (excavator/bobcat + operator) on the Job tab and re-price.'))
  }
  if (spec.pumpRequired && !items('machinery').some((i) => /pump/i.test(i.label))) {
    issues.push(issue('pump-not-costed', 'warn', 'Pump flagged, not costed', "You've marked this job as needing a concrete pump, but there's no pump in the price. Pump hire is a real day rate — don't wear it yourself.", 'Add the concrete pump hire, or clear the pump flag if access can chute.'))
  }

  /* ── GST on a client-ready quote ──────────────────────────────── */
  if (est.expected > 0 && est.gst <= 0) {
    issues.push(issue('no-gst', 'block', 'No GST on the quote', "The client-facing total has no GST on it. If you're registered, the price that goes out must include GST or you're 10% short on every job.", 'Confirm GST is applied before this goes to the client.'))
  }

  /* ── Margin ───────────────────────────────────────────────────── */
  if (est.marginPct <= 0) {
    issues.push(issue('zero-margin', 'block', 'No margin', "There's no profit margin on this quote — you'd be working for cost. One thing goes wrong and you're paying to do the job.", 'Set a margin on the Costs tab before this goes out.'))
  } else if (est.marginPct < lowMargin) {
    issues.push(issue('low-margin', 'warn', `Thin margin (${est.marginPct}%)`, `Margin is only ${est.marginPct}%. That leaves no room for the things that always come up. Make sure it's a deliberate call, not an accident.`, "Lift the margin on the Costs tab, or confirm you're cutting it on purpose."))
  }

  /* ── Overhead recovery (proxy: are we on starter rates?) ──────── */
  if (ctx.businessConfigured === false) {
    issues.push(issue('overhead-starter-rates', 'warn', 'Quoting on starter rates', "This is priced on generic starter rates, not your numbers. Your overheads — insurance, super, admin, tooling, ute — aren't in here, so you may be under-recovering on every quote.", 'Set up your Business profile so quotes carry your real rates and overheads.'))
  }

  /* ── Deposit on a larger quote ────────────────────────────────── */
  if (est.expected >= depositThreshold) {
    issues.push(issue('no-deposit', 'warn', 'Big job, no deposit terms', `This is a ${fmt(est.expected)} job. Funding materials and labour out of your own pocket on a job this size is how cash flow kills contractors. Take a deposit.`, 'Add deposit/progress-claim terms before this goes to the client.'))
  }

  /* ── Risky assumptions ────────────────────────────────────────── */
  if (spec.soil === 'unknown' && (spec.prepRequired || spec.excavationDepthMm > 0 || isConcrete)) {
    issues.push(issue('soil-unknown', 'warn', 'Ground not confirmed', "The soil type is still unknown. Reactive clay, rock or fill changes the dig, the disposal and the price. Don't lock a fixed price on a guess.", 'Confirm the ground (or quote it as an estimate with a site-visit note).'))
  }
  if (!spec.location || resolveLocation(spec.location).name === 'Unknown location') {
    issues.push(issue('location-unknown', 'warn', 'Site location not pinned', "I can't pin the town, so concrete freight and crew travel are assumed. In remote areas that's thousands of dollars of difference.", 'Set the location on the Job tab so freight and travel price correctly.'))
  }
  if (isConcrete && spec.access === 'difficult' && !spec.pumpRequired) {
    issues.push(issue('access-difficult-no-pump', 'warn', 'Tough access, no pump', "Access is difficult but there's no pump allowed for. Barrowing concrete a long way blows your labour out fast.", 'Price a pump or confirm the truck can get close enough to chute.'))
  }
  if (est.confidence < lowConfidence) {
    issues.push(issue('low-confidence', 'info', `Low confidence (${est.confidence}%)`, `I'm only ${est.confidence}% confident in these inputs. That usually means something key isn't nailed down yet.`, 'Tighten the inputs on the Job tab, or send it as an estimate — not a fixed price.'))
  }
  if (!spec.rawDescription.trim() && !spec.notes.trim()) {
    issues.push(issue('no-scope-notes', 'info', 'No scope notes', "There's no description or scope notes on this quote. A bare quote with no scope is where disputes start — write down what's in and what's out.", 'Add a short scope/inclusions note on the Job tab.'))
  }
...
function issue(id: string, severity: ForemanSeverity, title: string, detail: string, fix: string): ForemanIssue {
